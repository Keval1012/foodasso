...
import { MdDeliveryDining, MdOutlineWatchLater } from "react-icons/md";
import Dine from "../../../Styles/assets/img/delivery.svg";
...
const DelivaryCard: React.FC<ViewOrderProps> = ({ liveOrderList }) => {
  const navigate = useNavigate();
  const [delivaryOrderDetails, setDelivaryOrderDetails] = useState([]);
  ...
  const handleLiveAllOrderDetailId = async (id: any) => {
    if (id) {
      try {
        ...
        if (res?.status === 200) {
          setDelivaryOrderDetails(res.data?.data);
        }
      } catch (error) {}
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {liveordersata &&
          liveordersata.map((order: any) => (
            <div
              className="w-full bg-white border rounded-lg shadow-md mt-10"
              key={order?.id}
            >
              <div className="border-t border-b border-gray-200">
                <div className="flex items-center justify-between mb-4 bg-[#C9BBE7] rounded-t-lg p-3">
                  ...
                  <div className="relative">
                    <div className="absolute bottom-3 right-9 flex justify-center items-center bg-purple-400 rounded-full w-14 h-14">
                ...
                <div className="flex items-center justify-between mb-4 px-4">
                  ...
                  {/* <p className="text-sm text-gray-500">(Order Note: No onion garlic)</p> */}
                </div>
      ...
      {isMoreInfoDialogOpen && (
        <MoreInfoDialog
          ...
          viewOrderDetails={delivaryOrderDetails}
        />
      )}
