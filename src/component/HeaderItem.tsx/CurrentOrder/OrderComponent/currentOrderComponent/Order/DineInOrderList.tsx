...
import { setDineInOrderAllOpeData } from "../../../../../../redux/Features/OperationDataSlice";
...
const DineInOrderList: React.FC<OrderTypeDataProps> = ({
  selectedTab,
}) => {
  ...
  // const totalPages = Math.ceil(DineInOrderList.length / itemsPerPage);

  // const startIndex = (currentPage - 1) * itemsPerPage;
  // const endIndex = startIndex + itemsPerPage;
  // const paginatedData = DineInOrderList.slice(startIndex, endIndex);
  ...
  const totalItems = dineInOrderAllOpeData?.total_count;
  ...
  //       if (res?.status === 200) {
  //         dispatch(setDineInOrderAllOpeData(res.data));
  ...
  console.log("dineInOrderAllOpeData", dineInOrderAllOpeData);
      ...
      <div className="">
        <div className="table-container overflow-x-auto max-h-96">
          <table className="min-w-full bg-white text-sm border border-gray-200">
            ...
            <tbody>
              {dineInOrderAllOpeData?.data?.map((order: any) => (
                <tr
                  key={order?.id}
