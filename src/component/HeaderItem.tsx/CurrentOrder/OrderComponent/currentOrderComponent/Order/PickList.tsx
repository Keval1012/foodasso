...
import { setPickUpOrderAllOpeData } from "../../../../../../redux/Features/OperationDataSlice";
...
const PickList: React.FC<OrderTypeDataProps> = ({ selectedTab }) => {

  const navigate = useNavigate();
  const dispatch = useDispatch();
  const { loginUserData } =
    useSelector((state: any) => state.billingData) ?? {};
  const { pickUpOrderAllOpeData } =
  ...
  // const paginatedData = PickListData.slice(startIndex, endIndex);
  ...
  const totalItems = pickUpOrderAllOpeData?.total_count;
  ...
  //       if (res?.status === 200) {
  //         dispatch(setPickUpOrderAllOpeData(res.data));
  ...
  console.log("pickUpOrderAllOpeData", pickUpOrderAllOpeData);
      ...
      <div className="">
        <div className="table-container overflow-x-auto max-h-96">
          <table className="min-w-full bg-white text-sm border border-gray-200">
            ...
            <tbody>
              {pickUpOrderAllOpeData?.data?.map((order: any) => (
                <tr
                  key={order?.id}
