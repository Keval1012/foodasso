...
const Delivary: React.FC<TableDataProps> = ({
  tableId,
  tableDetails,
  areaType,
  allMenuData,
...
//     if(!tableWiseOrderData?.customer_details){
// return alert("Please fill user mobile no")
//     }
    setDialogOpen(true);}
...
console.log(defaultPaymentType,"tableWiseOrderDatatableWiseOrderData")
  ...
  const totalDiscountFunc = () => {
    ...
    if (subTotalPrice !== 0) {
      if (Number(subTotalPrice) <= Number(totalDiscount)) return 'Discount is not applied.';
    } else return 0;
  };

  const subTotalTaxFunc = () => {
    if (Number(subTotalPrice) > Number(totalDiscount)) return (subTotalPrice - totalDiscount);
    if (Number(subTotalPrice) <= Number(totalDiscount)) return Number(subTotalPrice);
  };
  ...
  return (
    <div className="flex flex-col justify-between">
      ...
      <div className="relative">
        ...
        {paymentTypeList.map((payType: any, index: number) => (
  <label className="flex items-center justify-center bg-white rounded-lg text-gray-800" key={payType?.type}>
    <input
      type="radio"
      name="payment"
      className="form-radio text-gray-800 mr-2"
      onClick={() => orderTypeModal(payType)}
      // If defaultPaymentType is not found, check the first radio button by default
    />
    {payType?.type}
  </label>
))}
