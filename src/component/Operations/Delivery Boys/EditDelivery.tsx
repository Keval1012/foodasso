...
interface defaultDeliveryBoyProps {
  defaultDeliveryBoy: any;
};
...
const EditDelivery = () => {
  ...
  const { defaultDeliveryBoy } = (location.state as defaultDeliveryBoyProps) || {};
  const { loginUserData } =
    useSelector((state: any) => state.billingData) ?? {};
  const [fullname, setFullname] = useState(defaultDeliveryBoy?.fullname);
  const [username, setUsername] = useState(defaultDeliveryBoy?.username);
  const [userCode, setUserCode] = useState(defaultDeliveryBoy?.user_code);
  const [password, setPassword] = useState(defaultDeliveryBoy?.password);
  const [phoneNumber, setPhoneNumber] = useState(defaultDeliveryBoy?.phone);
      ...
      <div className="mt-6 w-full">
        <div className="">
          ...
          <form
            ...
            className="bg-white m-4  rounded-lg border w-[875px]"
          >
            {/* Language */}
            ...
            {/* Buttons */}
            <div className="flex justify-end gap-6 border-t border-gray-300">
              <button
                type="button"
